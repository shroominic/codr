...
async def commit_changes(stage: bool, auto_push: bool, codebase: CodeBase) -> None:
    if stage:
        await codebase.bash("git add .")
    git_status = (await codebase.bash("git status")).split("Changes not staged for commit:")[0]
    if "Changes to be committed" in git_status:
        commits = await asyncio.gather(
            *[process_change(change, codebase) for change in git_status.split("\n") if change.startswith("\t")]
        )
        for change, msg in commits:
            await codebase.bash(f'git commit {change} -m "{msg}"')
            print(change, ">", msg)
        if auto_push:
            await codebase.bash("git push")


async def process_change(change: str, codebase: CodeBase) -> tuple[str, str]:
    change_split = change.split()
    if len(change_split) > 1:
        if change_split[0] == "modified:":
            file_change = change_split[1].strip()
            modifications = await codebase.bash(f"git diff --staged {file_change}")
