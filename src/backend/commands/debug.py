...
async def auto_debug(
    ...
    result = await analyze_output(codebase, command, goal)
    ...
    from .implement import implement_task

    await implement_task(codebase, description)

    if loop:
        print("DEBUGGING LOOP:", command)
        await auto_debug(codebase, command, goal, loop)


async def analyze_output(codebase: CodeBase, command: str, goal: str) -> str:
    """
    ...
    async def stream_output(
        ...
        async for batch in codebase.stream_bash(command):
